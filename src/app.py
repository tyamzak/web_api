...
def after_response_main(**bodyInfo):
    ...
    if videofile != "videofile":
      ...
      shared_url = send_video_Box_sharedlink(videofile)
    ...
    #ビデオファイルが空だったら、元々の送られてきた画像を使う
      ...
      if version.parse(Image.PILLOW_VERSION) < version.parse("3.4"):
          ...
          img.save("/tmp/videos/anomaly.gif", save_all=True, append_images=frames, optimize=False, duration=166,loop=0)
          logger.debug("gif作成完了")
    
    accessToken = "xoxb-2123538440165-2178559596549-BPYjL6DV6LJXw1H2W0IrNiQa" #株式会社全日警のWebhookID
...
def send_video_Box_sharedlink(file_name):
    auth = JWTAuth.from_settings_file('/projects/821417016_35hhfp6t_config.json')
    client = Client(auth)
...
#print('File "{0}" has a size of {1} bytes'.format(file_info.name,file_info.size))
    ...
    new_file = client.folder(folder_id).upload_stream(stream, file_name.replace('/tmp/videos/',''))
    print('File "{0}" uploaded to Box with file ID {1}'.format(new_file.name, new_file.id))
    logger.debug('File "{0}" がBoxに file ID {1}でアップロードされました'.format(new_file.name, new_file.id))
    url = client.file(new_file.id).get_shared_link()
    print('ファイルの共有リンクは: {0}'.format(url))
    return url

if __name__ == '__main__':
    app.run(debug=True, host='0.0.0.0', port=5050, threaded=True) #run app in debug mode on port 5050
